...
**Why it can be negative.** Nothing pins `r` above zero. If the precautionary motive is strong enough — high `σ`, `ρ`, `γ`, or a tight constraint — households pile up so much capital that its marginal product falls *below* depreciation, so `r = MPK − δ < 0`. Saving then literally loses value over time, yet households still do it, because in this economy capital is the **only store of value**: there is no other way to carry resources into a rainy day. The rate is bounded below by **`−δ`** — as `r → −δ`, firms' capital demand `K = L·((r+δ)/α)^{1/(α−1)}` explodes, so the market always clears at some `r > −δ`. (In the code this is the `r_low = −δ + 1e-4` floor of the root-find.) You can watch `r*` cross into negative territory by pushing `σ` up in §6 — at `σ = 0.4`, `r* ≈ −0.1%`.

**Is it "the stock market"?** Only loosely. `r` is the return on *owning capital*, which is stock-market-like, but here there is a **single, risk-free asset**: there are no aggregate shocks, so capital's return is *certain* each period. So `r` behaves like a **safe real interest rate** (a bank deposit or an inflation-protected government bond), not like the volatile return on equities. There is no equity premium and no aggregate risk in the model; the *only* risk is the **idiosyncratic**, household-level income shock, which households cannot trade away. The stock-market analogy that does fit: it is as if everyone were forced to hold the same risk-free savings account, earning a return the economy sets collectively — and when everyone crowds in for safety, that return gets bid down, sometimes below zero.
